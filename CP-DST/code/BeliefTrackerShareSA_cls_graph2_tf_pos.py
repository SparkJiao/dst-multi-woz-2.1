...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.transformer2 = initialize_transformer(encoder_layers=2)
        self.extra_nbt = args.extra_nbt
        if self.extra_nbt:
            self.transformer3 = initialize_transformer(encoder_layers=2, add_position_embedding=True)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None, transfer_labels=None):
        ...
        graph_hidden = self.transformer2(hidden_states=graph_query, key=graph_key, value=graph_value)[0]
        ...
        if self.extra_nbt:
            ...
            hidden = self.transformer3(hidden, attention_mask=causal_mask)[0].view(slot_dim, ds, ts, -1)
