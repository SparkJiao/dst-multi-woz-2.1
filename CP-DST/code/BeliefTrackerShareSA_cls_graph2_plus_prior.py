...
class BeliefTracker(nn.Module):
    ...
    def initialize_slot_value_lookup(self, label_ids, slot_ids, slot_token_type_ids=None):
        ...
        _tmp_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        slots = _tmp_tokenizer.batch_decode(slot_ids, skip_special_tokens=True)
        assert len(slots) == 30, len(slots)
        prior_mask = [[0] * len(slots) for _ in range(len(slots))]
        for i, slot_a in enumerate(slots):
            d_a, s_a = slot_a.split('-')
            for j, slot_b in enumerate(slots):
                d_b, s_b = slot_b.split('-')
