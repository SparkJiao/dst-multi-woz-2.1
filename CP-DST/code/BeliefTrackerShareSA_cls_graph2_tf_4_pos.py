...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        # NBT
        position_ids = torch.arange(30, dtype=torch.long, device=self.device)
        causal_mask = position_ids[None, :].repeat(30, 1) <= position_ids[:, None]
        self.register_buffer("causal_mask", causal_mask.to(dtype=torch.long))
        self.transformer1 = initialize_transformer(encoder_layers=2, add_position_embedding=True)
        self.transformer2 = initialize_transformer(encoder_layers=1)
        self.transformer3 = initialize_transformer(encoder_layers=2)
        self.extra_nbt = args.extra_nbt
        if self.extra_nbt:
            self.transformer3 = initialize_transformer(encoder_layers=1, add_position_embedding=True)
            self.transformer3.embed_positions = self.transformer1.embed_positions
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None, transfer_labels=None):
        ...
        causal_mask = self.causal_mask[None, :ts, :ts].expand(slot_dim * ds, -1, -1)
        # Neural belief tracking
        hidden = self.transformer1(hidden, attention_mask=causal_mask)[0].view(slot_dim * bs, -1)
        ...
        graph_hidden = self.transformer2(hidden_states=graph_query, key=graph_key, value=graph_key)[0]

        graph_key = graph_key.reshape(ds, ts - 1, slot_dim, -1)
        graph_hidden = graph_hidden.reshape(ds, ts - 1, slot_dim, -1)
        graph_key[:, 1:] = graph_hidden[:, :-1]
        graph_key = graph_key.reshape(ds * (ts - 1), slot_dim, -1)
        graph_hidden = graph_hidden.reshape(ds * (ts - 1), slot_dim, -1)
        graph_hidden = self.transformer3(hidden_states=graph_hidden, key=graph_key, value=graph_value)[0]
        ...
        hidden = torch.cat([hidden[:, :, :1], graph_hidden], dim=2)

        # Slot attention supervision
        loss = 0
        # if self.graph_add_sup > 0:
        ...
        if self.extra_nbt:
            ...
            hidden = self.transformer4(hidden, attention_mask=causal_mask)[0].view(slot_dim, ds, ts, -1)
