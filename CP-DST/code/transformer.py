...
from transformers.models.bart.modeling_bart import BartConfig, BartPretrainedModel, BaseModelOutput, ACT2FN, BartLearnedPositionalEmbedding
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    if len(mask.size()) == 2:
        ...
        expanded_mask = mask[:, None, :, :].expand(bsz, 1, -1, src_len).to(dtype)
    else:
...
def initialize_transformer(
    ...
    return BartEncoder(config, add_position_embedding=add_position_embedding)
...
class BartEncoder(BartPretrainedModel, ABC):
    ...
    def __init__(self, config: BartConfig, add_position_embedding: bool = False):
...
class BartEncoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.self_attn = BartAttentionSimplify(
...
class BartAttentionSimplify(nn.Module):
