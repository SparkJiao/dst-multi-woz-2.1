...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        if self.fuse_type == 0:
            self.graph_project = nn.Linear(self.bert_output_dim * 2, self.bert_output_dim)
        elif self.fuse_type == 1:
            self.graph_project = nn.Sequential(
                nn.Linear(self.bert_output_dim, self.bert_output_dim * 4),
                nn.GELU(),
                nn.Linear(self.bert_output_dim * 4, self.bert_output_dim),
                nn.Dropout(self.hidden_dropout_prob)
            )
            self.graph_project_layer_norm = nn.LayerNorm(self.bert_output_dim)
        else:
            raise RuntimeError()
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None, transfer_labels=None):
        ...
        if self.fuse_type == 0:
            ...
            graph_hidden = self.graph_project(torch.cat([hidden[:, :, 1:], graph_hidden], dim=-1))
        elif self.fuse_type == 1:
            graph_hidden = self.graph_project(graph_hidden)
            graph_hidden = self.graph_project_layer_norm(graph_hidden + hidden[:, :, 1:])
        else:
