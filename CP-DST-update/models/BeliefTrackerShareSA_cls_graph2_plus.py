...
class BeliefTracker(BertPreTrainedModel):
    ...
    def get_no_grad_modules(self):
        ...
        #     res.append(self.slot_token_type_ids)
        # if self.inter_domain:
        #     res.append(self.inter_domain_mask)
        # if self.slot_res is not None:
        #     res.append(self.slot_res)
        res = {
            nn.ModuleList,
            nn.ModuleDict,
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None, transfer_labels=None):
        ...
        # for i in range(12):
        #     logger.info(all_attn_cache[i]["key"].dtype)
        #     if torch.isnan(all_attn_cache[i]["key"]).any() or torch.isinf(all_attn_cache[i]["key"]).any():
        #         raise ValueError("Nan detected in bert with Layer: {}".format(i))
        ...
        if torch.isnan(hidden).any() or torch.isinf(hidden).any():
            raise ValueError("NaN or Inf found in hidden states. Trigger 1.")

        # Neural belief tracking
        hidden = self.transformer(hidden, None).view(slot_dim * bs, -1)

        if torch.isnan(hidden).any() or torch.isinf(hidden).any():
            raise ValueError("NaN or Inf found in hidden states. Trigger 2.")
        ...
        if torch.isnan(value_hidden).any() or torch.isinf(value_hidden).any():
            raise ValueError("NaN or Inf found in hidden states. Trigger 3.")
        ...
        if torch.isnan(graph_value).any() or torch.isinf(graph_value).any():
            raise ValueError("NaN or Inf found in hidden states. Trigger 4.")
        ...
        if torch.isnan(graph_hidden).any() or torch.isinf(graph_hidden).any():
            raise ValueError("NaN or Inf found in hidden states. Trigger 5.")
        ...
        if torch.isnan(hidden).any() or torch.isinf(hidden).any():
            raise ValueError("NaN or Inf found in hidden states. Trigger 6.")
