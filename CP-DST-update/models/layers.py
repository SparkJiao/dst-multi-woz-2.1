...
from transformers.models.bert.modeling_bert import BertConfig, BertPreTrainedModel, ACT2FN, BertIntermediate, BertOutput
    ...
    def __init__(self, config: BertConfig, add_output=False, residual=False, do_layer_norm=False,
        ...
        if self.add_output2:
            ...
            if self.do_layer_norm2:
                self.w_LayerNorm = nn.LayerNorm(config.hidden_size, eps=1e-6)
...
class FeedForwardNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, act_fn=gelu, dropout: float = 0.1,
        ...
        self.add_residual = add_residual
        if self.add_layer_norm:
            self.LayerNorm = nn.LayerNorm(input_size, eps=1e-6)
...
class LinearSeq(nn.Module):
    ...
    def forward(self, x):
        ...
        return alpha.unsqueeze(1).bmm(x)
...
def masked_log_softmax_fp16(vector: torch.Tensor, mask: torch.Tensor, dim: int = -1) -> torch.Tensor:
    ...
    if mask is not None:
        ...
        vector = vector.float() + (mask + 1e-6).log()
