...
fp_train = open("../train-5-full-value-domain.tsv", "w")
fp_dev = open("../dev-5-full-value-domain.tsv", "w")
fp_test = open("../test-5-full-value-domain.tsv", "w")
...
domain_file_list = {dom: {'train': [], 'dev': [], 'test': []} for dom in EXPERIMENT_DOMAINS}
...
for file_id in tqdm(data, total=len(data)):
        ...
        if idx % 2 == 0:  # user turn
            ...
            for domain in data[file_id]['log'][idx]['metadata'].keys():
                ...
                for slot in data[file_id]['log'][idx]['metadata'][domain]['semi'].keys():
                    ...
                    # extract domain
                    if value == 'not mentioned' or value in ['dont care', 'dontcare', 'don\'t care', 'do not care']:
                        domain_active = True
                    ...
                    if value not in ontology[domain][slot] and value != 'none':
                        ...
                        # value = 'none'
                        # value = 'undefined'
                        data_ont[str(domain) + '-' + str(slot)].append(value)
                        ontology[domain][slot][value] = 1
                        undefined += 1
                ...
                for slot in data[file_id]['log'][idx]['metadata'][domain]['book'].keys():
                    ...
                    if value not in ontology[domain]['book ' + slot] and value != 'none':
                        ...
                        # value = 'none'
                        # value = 'undefined'
                        data_ont[str(domain) + '-book ' + str(slot)].append(value)
                        ontology[domain]['book ' + slot][value] = 1
                        undefined += 1

                    belief[str(domain) + '-book ' + str(slot)] = value

                if domain_active and domain not in active_domains:
    ...
    for dom in active_domains:
        if fp_out == fp_dev:
            ...
            split = 'train'
        domain_file_list[dom][split].append(file_id)
...
# with open('../ontology-full.json', 'w') as f:
#     json.dump(data_ont, f, indent=2)

for dom, split in domain_file_list.items():
    for split_name, split_ls in split.items():
        print(dom, split_name, len(split_ls))
