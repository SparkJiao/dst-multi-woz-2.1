...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, attn_cache=None):
        ...
        if attn_cache is not None and self.key_type != -1:
            """
            ...
            seq_att_scores.masked_fill_(attention_mask[:, :, :, :seq_length], NEG_INF)
            seq_att_scores = self.dropout(nn.Softmax(dim=-1)(seq_att_scores))
            seq_att_value = seq_att_scores.matmul(self.transpose_for_scores(attn_cache["value"]))
            ...
            #  2. Add a extra key linear projection for 1.
            # ================ 0 ================
            if self.key_type == 0:
                attention_scores = torch.cat([slot_att_scores, attention_scores], dim=-1)
            ...
            # =============== 1 =================
                ...
                attention_scores = torch.cat([extra_scores, attention_scores], dim=-1)
                value_layer = torch.cat([seq_att_value, value_layer], dim=2)
                attention_mask = torch.cat([extra_mask, attention_mask], dim=-1)
            elif self.key_type == 2:
                extra_scores = torch.matmul(query_layer, self.extra_key(seq_att_value).transpose(-1, -2))
                attention_scores = torch.cat([extra_scores, attention_scores], dim=-1)
                value_layer = torch.cat([seq_att_value, value_layer], dim=2)
                attention_mask = torch.cat([extra_mask, attention_mask], dim=-1)
            elif self.key_type == 3:
                # extra_scores = torch.matmul(query_layer, seq_att_value.transpose(-1, -2)).detach()
                extra_scores = torch.matmul(query_layer, self.extra_key(seq_att_value.detach()).transpose(-1, -2))
                attention_scores = torch.cat([extra_scores, attention_scores], dim=-1)
                value_layer = torch.cat([seq_att_value, value_layer], dim=2)
                attention_mask = torch.cat([extra_mask, attention_mask], dim=-1)
            else:
        ...
        attention_scores.masked_fill_(attention_mask, NEG_INF)
        attention_probs = nn.Softmax(dim=-1)(attention_scores)
        # attention_probs = masked_softmax(attention_scores, attention_mask, dim=-1)
...
def masked_softmax(vector: torch.Tensor, mask: torch.Tensor, dim: int = -1) -> torch.Tensor:
    if mask is None:
        ...
        result = result * mask
        result = result / (result.sum(dim=dim, keepdim=True) + 1e-6)
    return result
