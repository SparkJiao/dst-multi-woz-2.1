...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.dialog_fuse_layer = layers.FusionLayer(self.bert_output_dim, dropout=self.hidden_dropout_prob)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        # Dialog modeling
        dia_h = dia_hidden[:, 0].unsqueeze(0).expand(slot_dim, -1, -1).reshape(slot_dim * ds, ts, -1)
        ...
        hidden = self.dialog_fuse_layer(hidden, dia_h)
