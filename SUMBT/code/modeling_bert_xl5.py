...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.turn_embedding_query = nn.Linear(config.hidden_size, config.hidden_size)
        self.turn_embedding_key = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def generate_turn_embedding(self, pre_turn, device):
        ...
        turn_query = self.turn_embedding_query(turn_embedding[-1]).view(1, self.num_attention_heads, self.attention_head_size)
        turn_key = self.turn_embedding_key(turn_embedding).view(pre_turn + 1, self.num_attention_heads, self.attention_head_size)

        turn_scores = torch.einsum("ihd,phd->hip", [turn_query, turn_key])  # (num_head, 1, pre_turn + 1)
...
class BertReshapeAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, attn_cache=None, slot_dim=0, slot_unified_mask=None,
                use_context=False, pre_turn=0, max_turn=0, turn_scores=None):
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
        ...
        # Add turn scores
        if use_context and turn_scores is not None:
            turn_scores = self.expand_turn_score(turn_scores, hidden_states.size(0), hidden_states.size(1))
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, position_ids=None,
        ...
        embedding_output, turn_scores = self.embeddings(input_ids, token_type_ids, position_ids=position_ids,
                                                        start_offset=start_offset, pre_turn=pre_turn)

        if turn_scores is not None:
            kwargs["turn_scores"] = turn_scores
