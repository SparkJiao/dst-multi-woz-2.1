...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.transformer2 = SimpleDialogSelfAttention(nbt_config, add_output=True,
                                                      add_layer_norm=args.sa_add_layer_norm,
                                                      add_residual=args.sa_add_residual,
                                                      self_attention=nbt_ov_attn)
        self.transformer3 = SimpleDialogSelfAttention(nbt_config, add_output=True,
                                                      add_layer_norm=args.sa_add_layer_norm,
                                                      add_residual=args.sa_add_residual,
                                                      self_attention=nbt_ov_attn)
        self.transformer2.position_embeddings.weight = self.utterance_encoder.bert.embeddings.position_embeddings.weight
        self.transformer3.position_embeddings.weight = self.utterance_encoder.bert.embeddings.position_embeddings.weight
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        dia_h = self.transformer2(dia_h, None).view(bs, -1)
        ...
        sum_dia_h = self.transformer3(sum_dia_h, None).view(slot_dim, bs, -1)
