...
def get_flat_slot_embedding(slots, max_seq_length, tokenizer, device):
    all_input_ids = []
    other_zero_out_mask = []
    cls_pos = []
    for slot in slots:
        slot_tokens = ["[CLS]"] + tokenizer.tokenize(slot) + ["[SEP]"]
        if slot_tokens > max_seq_length:
            raise RuntimeError("Slot was truncated.")
        slot_token_ids = tokenizer.convert_tokens_to_ids(slot_tokens)

        cls_pos.append(len(all_input_ids))
        input_mask = [0] * len(all_input_ids)

        all_input_ids.extend(slot_token_ids)
        input_mask.extend([1] * len(slot_token_ids))

        other_zero_out_mask.append(input_mask)
    for input_mask in other_zero_out_mask:
        prefix_len = len(input_mask)
        input_mask.extend([0] * (len(all_input_ids) - prefix_len))

    all_input_ids = torch.LongTensor(all_input_ids, device=device)
    other_zero_out_mask = torch.LongTensor(other_zero_out_mask, device=device)
    cls_pos = torch.LongTensor(cls_pos, device=device)
    return all_input_ids, other_zero_out_mask, cls_pos
