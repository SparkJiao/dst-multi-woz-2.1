            ...
            if not config.target_slot == 'all':
                ...
                for key, value in slot_idx.items():
                    if key != config.target_slot:
                        ...
                        target_domains.append(key)
                config.target_slot = ':'.join(target_slot)
            else:
                target_domains = slot_idx.keys()

            domain_len = []
            for d in target_domains:
                slot_ids = slot_idx[d].split(':')
                domain_len.append(len(slot_ids))
            start = 0
            domain_spans = []
            for d_length in domain_len:
                domain_spans.append((start, start + d_length))
                start += d_length
...
def main():
    ...
    # Set domain slots span
    model.set_domain_spans(processor.domain_spans)
