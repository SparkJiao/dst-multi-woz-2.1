...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.ve_classifier = copy.deepcopy(self.classifier)
        self.ve_hidden_output = copy.deepcopy(self.hidden_output)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        ve_answer_type_logits = self.ve_classifier(re_hidden_d).view(slot_dim, ds, ts, -1)

        # _, answer_type_pred = answer_type_logits.max(dim=-1)
        _, answer_type_pred = ((answer_type_logits.detach() + ve_answer_type_logits.detach()) / 2.).max(dim=-1)

        hidden = self.hidden_output(hidden).view(slot_dim, ds, ts, -1)
        re_hidden_d = self.ve_hidden_output(re_hidden_d).view(slot_dim, ds, ts, -1)
        ...
        for s, slot_id in enumerate(target_slot):  # note: target_slots are successive
            ...
            if self.distance_metric == 'product':
                ...
                _ve_hidden = re_hidden_d[s].unsqueeze(2).view(ds * ts, 1, -1)
                _ve_dist = self.metric(_ve_hidden, _hid_label).squeeze(1).reshape(ds, ts, num_slot_labels)
            else:
                _hid_label = hid_label.unsqueeze(0).unsqueeze(0).repeat(ds, ts, 1, 1).view(ds * ts * num_slot_labels, -1)
                ...
                _ve_hidden = re_hidden_d[s].unsqueeze(2).repeat(1, 1, num_slot_labels, 1).view(
                    ds * ts * num_slot_labels, -1)
                _ve_dist = self.metric(_hid_label, _ve_hidden).view(ds, ts, num_slot_labels)

            if self.distance_metric == "euclidean":
                _dist = -_dist
                _ve_dist = -_ve_dist
            # _, pred = torch.max(_dist, -1)
            cb_dist = (_dist.detach() + _ve_dist.detach()) / 2.
            _, pred = torch.max(cb_dist, dim=-1)
            pred_slot.append(pred.view(ds, ts, 1))
            # output.append(_dist)
            output.append(cb_dist)
            ...
            if labels is not None:
                ...
                _loss = self.nll(_ve_dist.view(ds * ts, -1), masked_slot_labels_for_loss.view(-1)) / (ds * 1.0)
                loss_slot[-1] += _loss.item()
                matching_loss += _loss.item()
                loss += _loss
            ...
            if answer_type_ids is not None:
                ...
                cls_loss = self.nll(ve_answer_type_logits[s].view(ds * ts, -1), answer_type_ids[:, :, s].view(-1)) / ds
                loss_slot[-1] += cls_loss.item()
                cls_loss = cls_loss * self.cls_loss_weight
                type_loss += cls_loss.item()
                loss += cls_loss
