...
class BertLayer(nn.Module):
    ...
    def copy_weight(self):
        if hasattr(self, "slot_attention"):
            self.slot_attention = copy.deepcopy(self.attention)
        if hasattr(self, "flow"):
            self.flow = copy.deepcopy(self.attention)
    ...
    def share_weight(self):
        ...
        if hasattr(self, "flow"):
            self.flow.self.query.weight = self.attention.self.query.weight
            self.flow.self.key.weight = self.attention.self.query.weight
            self.flow.self.value.weight = self.attention.self.value.weight
            self.flow.output.dense.weight = self.attention.output.dense.weight

    def full_share(self):
        if hasattr(self, "slot_attention"):
            self.slot_attention = self.attention
        if hasattr(self, "flow"):
            self.flow = self.attention
    ...
    def forward(self, hidden_states, attention_mask, attn_cache=None, do_slot_attention: bool = False,
        ...
        if use_flow:
            hidden_states = self._flow_op(hidden_states, dialog_mask)
