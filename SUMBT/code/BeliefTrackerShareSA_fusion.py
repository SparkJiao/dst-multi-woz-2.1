...
try:
    ...
    from .global_logger import get_child_logger, get_summary_writer
except ImportError:
    import layers
    from modeling_bert_extended import BertModel, SimpleDialogSelfAttention, SimpleSelfAttention
    from global_logger import get_child_logger, get_summary_writer
...
summary_writer: SummaryWriter = get_summary_writer()
...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.slot_attention = layers.MultiHeadAttention(nbt_config)
        self.dynamic_fuse = layers.DynamicFusion(self.bert_output_dim)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        if self.slot_token_type_ids is not None:
            slot_token_type_ids = self.slot_token_type_ids.unsqueeze(1).expand(-1, bs, -1).reshape(-1,
        ...
        # summary_writer.add_histogram(f"{'Train' if self.training else 'Validate'}/gate", gate_tensor)

        # NBT
        hidden = self.transformer(hidden, None).view(slot_dim, bs, -1).transpose(0, 1)

        slot_diag_mask = torch.diag(torch.zeros(slot_dim, dtype=next(self.parameters()).dtype,
                                                device=self.device).fill_(-10000.0), diagonal=0)[None, None, :, :]
        slot_hidden, _ = self.slot_attention(hidden, hidden, hidden, attention_mask=slot_diag_mask)

        hidden = self.dynamic_fuse(hidden, slot_hidden).transpose(0, 1).reshape(slot_dim, ds, ts, -1)
        ...
        if n_gpu == 1:
            ...
            return loss.unsqueeze(0), None, acc.unsqueeze(0), type_acc.unusqueeze(0), acc_slot.unsqueeze(
                0), type_acc_slot.unsqueeze(0), torch.cat([
