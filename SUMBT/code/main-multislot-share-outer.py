...
def main():
    ...
    parser.add_argument('--slot_step', default=1, type=int)
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", dynamic_ncols=True)):
                ...
                acc_slot = []
                pred_slot = []
                for slot_step in range(0, slot_num, args.slot_step):
                    target_slot = list(range(slot_step, slot_step + args.slot_step))
                    # Forward
                    if n_gpu == 1:
                        loss, _loss_slot, _acc, _acc_slot, _pred_slot = model(input_ids, token_type_ids, input_mask,
                                                                        label_ids[:, :, target_slot], n_gpu, target_slot=target_slot)
                    else:
                        loss, _, _acc, _acc_slot, _pred_slot = model(input_ids, token_type_ids, input_mask,
                                                               label_ids[:, :, target_slot], n_gpu, target_slot=target_slot)

                        # average to multi-gpus
                        loss = loss.mean()
                        _acc = _acc.mean()
                        _acc_slot = _acc_slot.mean(0)
                    ...
                    loss_slot.extend(_loss_slot)
                    acc_slot.extend(_acc_slot.detach().cpu().tolist())
                    pred_slot.append(pred_slot)
                    tr_loss += loss.item()

                pred_slot = torch.cat(pred_slot, dim=-1)
                joint_acc, _, num_turn, _ = _eval_acc(pred_slot, label_ids)
                acc = (joint_acc / num_turn).item()
