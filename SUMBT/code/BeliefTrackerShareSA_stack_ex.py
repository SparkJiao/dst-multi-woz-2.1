...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.extra_dropout = args.extra_dropout
        logger.info(f"Self.extra_dropout: {self.extra_dropout}")
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        for layer_idx, attn_cache in enumerate(mid_hidden_cache):
            ...
            all_attn_cache[layer_idx].append(new_cache)

        slot_mask = torch.cat([
            attention_mask.unsqueeze(0).expand(slot_dim, -1, -1, -1).reshape(
                slot_dim * bs, self.max_seq_length).to(self.slot_mask.dtype),
            self.slot_mask[None, None, :, :].expand(slot_dim, bs, -1, -1).reshape(slot_dim * bs, slot_dim * self.max_slot_length),
            ini_slot_mask
        ], dim=-1)
        ...
        hidden, _, _ = self.utterance_encoder(slot_ids, token_type_ids=slot_token_type_ids, attention_mask=slot_mask,
                                              ...
                                              extra_dropout=self.extra_dropout)
