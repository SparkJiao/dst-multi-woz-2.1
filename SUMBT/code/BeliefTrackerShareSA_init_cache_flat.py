        ...
        for layer_idx, attn_cache in enumerate(all_attn_cache):
            for k in attn_cache:
                ...
                attn_cache[k] = torch.cat([
                    attn_cache[k],
                    slot_attn_cache[layer_idx][k].unsqueeze(0).expand(slot_dim, bs, -1, -1).reshape(slot_dim * bs, slot_dim * self.max_slot_length, self.bert_output_dim)
                ], dim=1)
        hidden, _, _ = self.utterance_encoder(slot_ids, token_type_ids=None, attention_mask=slot_mask,
                                              output_all_encoded_layers=False, start_offset=self.max_seq_length + slot_dim * self.max_slot_length,
