...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.dial_transformer = SimpleDialogSelfAttention(nbt_config, add_output=True,
                                                          add_layer_norm=args.sa_add_layer_norm,
                                                          add_residual=args.sa_add_residual,
                                                          self_attention=nbt_ov_attn)
        ...
        if args.share_position_weight:
            ...
            self.dial_transformer.position_embeddings.weight = self.utterance_encoder.bert.embeddings.position_embeddings.weight
        ...
        self.graph_attention = layers.DiagonalAttention(self.bert_output_dim * 2, multi_view_diag_attn_hidden_dim,
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        dia_h = self.dial_transformer(dia_h, None).view(bs, -1)
        dia_h = dia_h.unsqueeze(0).expand(slot_dim, -1, -1).contiguous()  # (slot_dim, bs, h)

        multi_view_hidden = torch.cat([hidden, dia_h], dim=-1).reshape(slot_dim, ds, ts, -1)
