...
try:
    from . import layers
    from .modeling_bert_extended1 import BertModel, SimpleDialogSelfAttention, SimpleSelfAttention
except ImportError:
    import layers
    from modeling_bert_extended1 import BertModel, SimpleDialogSelfAttention, SimpleSelfAttention
...
class BeliefTracker(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels, n_gpu=1, target_slot=None):
        ...
        slot_mask = self.slot_mask.unsqueeze(0).expand(bs, -1, -1).reshape(bs, total_len)
        slot_mask = torch.cat(
            [attention_mask.view(-1, self.max_seq_length).to(dtype=slot_ids.dtype), slot_mask], dim=-1)
