...
try:
    from . import layers
    from .modeling_bert import BertModel, SimpleDialogSelfAttention, SimpleSelfAttention, DialogReshape, TransposeBertEncoder
except ImportError:
    import layers
    from modeling_bert import BertModel, SimpleDialogSelfAttention, SimpleSelfAttention, DialogReshape, TransposeBertEncoder
...
class BertForUtteranceEncoding(BertPreTrainedModel):
    def __init__(self, config, reduce_layers: int = 0, dialog_reshape=DialogReshape(),
                 transpose_layer: int = -1):
        super(BertForUtteranceEncoding, self).__init__(config)
        print(f'Reduce {reduce_layers} of BERT.')
        config.num_hidden_layers = config.num_hidden_layers - reduce_layers
        config.dialog_reshape = dialog_reshape
        config.transpose_layer = transpose_layer
        self.config = config
        self.bert = BertModel(config)
        self.bert.encoder = TransposeBertEncoder(config)
...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.dialog_reshape = DialogReshape()
        self.transpose_layer = args.transpose_layer
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels, n_gpu=1, target_slot=None):
        ...
        self.utterance_encoder.bert.encoder.transpose_layer = -1
        ...
        concat_slot_mask = self.slot_mask.unsqueeze(1).expand(-1, bs, -1).reshape(-1, self.max_slot_length)
        concat_slot_mask = torch.cat(
            [attention_mask.unsqueeze(0).expand(slot_dim, -1, -1, -1).reshape(-1, self.max_seq_length),
             concat_slot_mask], dim=-1)
        self.dialog_reshape.set_size(slot_dim, ds, ts, self.max_slot_length, concat_slot_mask)
        ...
        self.utterance_encoder.bert.encoder.transpose_layer = self.transpose_layer
        hidden, _, _ = self.utterance_encoder(self.slot_ids, token_type_ids=None, attention_mask=self.slot_mask,
