...
try:
    from . import layers
    from .modeling_bert import HalfBert, HalfDialogTransformer
except ImportError:
    import layers
    from modeling_bert import HalfBert, HalfDialogTransformer
...
class BertForUtteranceEncoding(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False,
        ...
        return self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers,
                         start_offset=start_offset, all_attn_cache=all_attn_cache, start_layer=start_layer, end_layer=end_layer)
...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.transformer = HalfDialogTransformer(nbt_config, bert_encoder=self.utterance_encoder.bert.encoder)
    ...
    def forward(self, input_ids, input_len, slot_gates, labels, n_gpu=1, target_slot=None):
        ...
        hidden, _, _ = self.utterance_encoder(slot_ids, token_type_ids=None, attention_mask=slot_mask,
                                              ...
                                              all_attn_cache=all_attn_cache, start_layer=0, end_layer=10)
        hidden = hidden[:, 0].view(slot_dim, ds, ts, -1).view(-1, ts, self.bert_output_dim)

        # NBT
        hidden = self.transformer(hidden, None, False, start_layer=10, end_layer=12).view(slot_dim, ds, ts, -1)

        slot_gate_logits = self.classifier(hidden)
        _, slot_gate_pred = slot_gate_logits.max(dim=-1)

        # Label (slot-value) encoding
        # loss = self.nll(slot_gate_logits.view(-1, 3), slot_gates.permute(2, 0, 1).view(-1))
        loss = 0
        loss_slot = []
        pred_slot = []
        output = []
        for s, slot_id in enumerate(target_slot):
            # loss calculation
            ...
            pred_slot.append(pred.view(ds, ts, 1).detach())
            output.append(_dist)
            ...
            if slot_gates is not None:
                cls_loss = self.nll(slot_gate_logits[s].view(ds * ts, -1), slot_gates[:, :, s].view(-1))
        ...
        slot_gate_pred = slot_gate_pred.permute(1, 2, 0).detach()  # (ds, ts, slot_dim)
        classified_mask = ((slot_gates != 2) * (slot_gates != -1)).view(-1, slot_dim)  # 1 for `none` and `do not care`
        # mask classifying values as 1
        value_accuracy = (pred_slot == labels).view(-1, slot_dim).masked_fill(classified_mask, 1)
        slot_gate_accuracy = (slot_gate_pred == slot_gates).view(-1, slot_dim)
        ...
        # Slot accuracy
        acc_slot = accuracy.sum(dim=0).float() / (slot_gates.view(-1, slot_dim) > -1).float().sum(dim=0)
        # Joint accuracy
        acc = (accuracy.sum(dim=1) / slot_dim).sum().float() / (slot_gates[:, :, 0].view(-1) > -1).sum(dim=0).float()

        if n_gpu == 1:
            return loss, loss_slot, acc, acc_slot, torch.cat([slot_gate_pred.unsqueeze(-1), pred_slot.unsqueeze(-1)], dim=-1)
        else:
            return loss.unsqueeze(0), None, acc.unsqueeze(0), acc_slot.unsqueeze(0), torch.cat([
                slot_gate_pred.unsqueeze(-1), pred_slot.unsqueeze(-1)], dim=-1).unsqueeze(0)
