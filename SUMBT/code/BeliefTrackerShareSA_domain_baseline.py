...
class BeliefTracker(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        domain_hidden = hidden[:, 1].view(slot_dim * ds, ts, -1)
        hidden = hidden[:, 0].view(slot_dim * ds, ts, -1)

        # NBT
        hidden = self.transformer(hidden, None).view(slot_dim, ds, ts, -1)
        domain_hidden = self.domain_transformer(domain_hidden).view(slot_dim, ds, ts, -1)

        domain_h = []
        ex_hidden = []
        domain_labels = []
        for (domain_s, domain_e) in self.domain_spans:
            _domain_h = domain_hidden[domain_s:domain_e].mean(dim=0)
            domain_h.append(_domain_h.unsqueeze(0))
            ex_hidden.append(_domain_h.unsqueeze(0).expand(domain_e - domain_s, -1, -1, -1))
            domain_label = labels[:, :, domain_s:domain_e].sum(dim=-1)
            domain_label.clamp_(-1, 1)
            # domain_label[domain_label > 0] = 1
            # domain_label[domain_label == 0] = 0
            # domain_label[domain_label < 0] = -1
            # domain_label = (labels[:, :, domain_s:domain_e].sum(dim=-1) > 0).to(dtype=torch.long)
