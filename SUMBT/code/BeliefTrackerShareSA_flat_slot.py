...
try:
    from . import layers
    from .modeling_bert_layers import BertModel, SimpleDialogSelfAttention, SimpleSelfAttention
except ImportError:
    import layers
    from modeling_bert_layers import BertModel, SimpleDialogSelfAttention, SimpleSelfAttention
...
class BeliefTracker(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels, n_gpu=1, target_slot=None):
        ...
        initial_slot_mask = slot_mask.to(dtype=next(self.parameters()).dtype)
        initial_slot_mask = (1.0 - initial_slot_mask) * -10000.0
        slot_mask = torch.cat(
        ...
        hidden, _, _ = self.utterance_encoder(slot_ids, token_type_ids=None, attention_mask=slot_mask,
                                              ...
                                              all_attn_cache=all_attn_cache, do_slot_attention=True, slot_mask=initial_slot_mask,
                                              slot_dim=slot_dim)
