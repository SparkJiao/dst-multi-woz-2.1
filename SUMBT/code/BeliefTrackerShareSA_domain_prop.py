...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        if args.override_attn:
            ...
            self.domain_transformer = SimpleDialogSelfAttention(nbt_config, add_output=True,
                                                                add_layer_norm=args.sa_add_layer_norm,
                                                                self_attention=last_attention)
        else:
            self.transformer = SimpleDialogSelfAttention(nbt_config, add_output=True,
        ...
        if args.share_position_weight:
            ...
            self.domain_transformer.position_embeddings.weight = self.utterance_encoder.bert.embeddings.position_embeddings.weight

        self.classifier = layers.ProjectionTransform(self.bert_output_dim, num_classes=3)
        self.domain_classifier = nn.Sequential(nn.Linear(self.bert_output_dim, self.bert_output_dim),
                                               nn.Tanh(),
                                               nn.Linear(self.bert_output_dim, 2))
    ...
    def initialize_slot_value_lookup(self, label_ids, slot_ids, slot_token_type_ids=None):
        ...
        # domain_ids = slot_ids[:, :3]
        # domain_input_mask = domain_ids.new_ones(domain_ids.size())
        # self.register_buffer("domain_ids", domain_ids)
        # self.register_buffer("domain_input_mask", domain_input_mask)
        # self.register_buffer("domain_type_ids", domain_input_mask)
    ...
    def set_domain_spans(self, domain_spans):
        ...
        domain_mask = torch.zeros((len(domain_spans), self.slot_ids.size(0), self.max_slot_length),
                                  dtype=torch.long, device=self.device)
        unit_ones = torch.ones((1, self.max_slot_length), dtype=torch.long, device=self.device)
        for domain_idx, (domain_s, domain_e) in enumerate(self.domain_spans):
            domain_mask[domain_idx][domain_s:domain_e] = unit_ones.expand(domain_e - domain_s, -1)
        self.register_buffer('domain_mask', domain_mask)

        domain_ids = self.slot_ids[[span[0] for span in domain_spans], :3]
        domain_input_mask = (domain_ids > 0).to(dtype=torch.long)
        self.register_buffer("domain_ids", domain_ids)
        self.register_buffer("domain_input_mask", domain_input_mask)
        self.register_buffer("domain_type_ids", domain_input_mask)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None):
        ...
        hidden, _, slot_cache = self.utterance_encoder(slot_ids, token_type_ids=slot_token_type_ids, attention_mask=slot_mask,
                                                       output_all_encoded_layers=False, all_attn_cache=all_attn_cache,
                                                       start_offset=self.max_seq_length, slot_dim=slot_dim)

        num_domain = self.domain_ids.size(0)
        domain_ids = self.domain_ids.unsqueeze(1).expand(-1, bs, -1).reshape(-1, 3)  # (num_domain, bs, 3)
        domain_type_ids = self.domain_type_ids.unsqueeze(1).expand(-1, bs, -1).reshape(-1, 3)
        domain_mask = self.domain_mask * self.slot_mask.unsqueeze(0)  # (num_domain, slot_dim, max_slot_length)
        domain_mask = torch.cat([
            domain_mask.reshape(num_domain, slot_dim * self.max_slot_length),
            self.domain_input_mask
        ], dim=1).unsqueeze(1).expand(-1, bs, -1).reshape(num_domain * bs, slot_dim * self.max_slot_length + 3)

        for layer_idx, layer_slot_cache in enumerate(slot_cache):
            num_head = layer_slot_cache[0]["key"].size(1)
            for k in layer_slot_cache[0].keys():
                layer_slot_cache[0][k] = layer_slot_cache[0][k].view(
                    slot_dim, bs, num_head, self.max_slot_length, self.bert_output_dim // num_head).permute(1, 2, 0, 3, 4).reshape(
                    bs, num_head, slot_dim * self.max_slot_length, self.bert_output_dim // num_head)

        domain_hidden, _, _ = self.utterance_encoder(domain_ids, token_type_ids=domain_type_ids, attention_mask=domain_mask,
                                                     output_all_encoded_layers=False, all_attn_cache=slot_cache,
                                                     start_offset=self.max_seq_length + self.max_slot_length, slot_dim=num_domain)

        hidden = hidden[:, 0].view(slot_dim * ds, ts, -1)
        domain_hidden = domain_hidden[:, 1].view(num_domain * ds, ts, -1)

        # NBT
        hidden = self.transformer(hidden, None).view(slot_dim, ds, ts, -1)
        domain_hidden = self.domain_transformer(domain_hidden, None).view(num_domain, ds, ts, -1)

        ex_hidden = []
        domain_labels = []
        for domain_idx, (domain_s, domain_e) in enumerate(self.domain_spans):
            ex_hidden.append(domain_hidden[domain_idx].unsqueeze(0).expand(domain_e - domain_s, -1, -1, -1))
            domain_label = labels[:, :, domain_s:domain_e].sum(dim=-1)
            domain_label.clamp_(-1, 1)
            domain_labels.append(domain_label.unsqueeze(0))
        domain_labels = torch.cat(domain_labels, dim=0).view(-1)
        ex_hidden = torch.cat(ex_hidden, dim=0)
