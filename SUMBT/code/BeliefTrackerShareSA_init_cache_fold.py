...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        if args.across_slot:
            logger.info("Add cross slot attention")
            logger.info(f"Cross slot attention add layer norm: {args.ss_add_layer_norm}")
    ...
    def initialize_slot_value_lookup(self, label_ids, slot_ids):
        ...
        slot_len = torch.arange(slot_dim, dtype=torch.long, device=slot_ids.device)
        ...
        self.register_buffer("flat_slot_type_ids", flat_slot_type_ids.contiguous())
        self.register_buffer("fold_slot_type_ids", fold_slot_type_ids.contiguous())
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels, n_gpu=1, target_slot=None):
        ...
        _, _, slot_attn_cache = self.utterance_encoder(slot_ids, token_type_ids=slot_type_ids,
                                                       attention_mask=slot_mask_cache,
                                                       output_all_encoded_layers=False,
                                                       start_offset=self.max_seq_length)
        ...
        slot_mask = torch.cat(
            [attention_mask.unsqueeze(0).expand(slot_dim, -1, -1, -1).reshape(-1, self.max_seq_length),
             slot_mask, slot_mask], dim=-1)
        assert slot_ids.size(0) == slot_dim * bs
        for layer_idx, attn_cache in enumerate(all_attn_cache):
            for k in attn_cache:
                attn_cache[k] = attn_cache[k].unsqueeze(0).expand(slot_dim, -1, -1, -1) \
                    .reshape(slot_dim * bs, self.max_seq_length, -1)
                attn_cache[k] = torch.cat([
                    attn_cache[k],
                    slot_attn_cache[layer_idx][k].view(slot_dim, self.max_slot_length, self.bert_output_dim).\
                        unsqueeze(1).expand(-1, bs, -1, -1).reshape(-1, self.max_slot_length, self.bert_output_dim)
                ], dim=1)
        hidden, _, _ = self.utterance_encoder(slot_ids, token_type_ids=None, attention_mask=slot_mask,
                                              output_all_encoded_layers=False, start_offset=self.max_seq_length + self.max_slot_length,
