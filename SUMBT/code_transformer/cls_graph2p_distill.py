...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.transformer = SimpleDialogSelfAttention(nbt_config, add_layer_norm=args.sa_add_layer_norm, add_residual=args.sa_add_residual)
        if args.share_position_weight:
            logger.info("Dialog self attention will share position embeddings with BERT")
            self.transformer.tie_position_embedding(self.utterance_encoder.distilbert.embeddings.position_embeddings)

        logger.info(f"If extra neural belief tracker: {args.extra_nbt}")
        self.extra_nbt = args.extra_nbt
        if self.extra_nbt:
            nbt_config.n_heads = args.extra_nbt_attn_head

            self.belief_tracker = SimpleDialogSelfAttention(nbt_config, add_layer_norm=args.sa_add_layer_norm,
                                                            add_residual=args.sa_add_residual)
            if args.share_position_weight:
                self.belief_tracker.tie_position_embedding(self.utterance_encoder.distilbert.embeddings.position_embeddings)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, answer_type_ids, labels, n_gpu=1, target_slot=None, transfer_labels=None):
        ...
        # Utterance encoding
        all_attn_cache = self.utterance_encoder(input_ids=input_ids.view(-1, self.max_seq_length),
                                                attention_mask=attention_mask.view(-1, self.max_seq_length))[1]
        ...
        for i in range(self.utterance_encoder.config.n_layers):
            ...
            all_attn_cache[i]["value"] = all_attn_cache[i]["value"].unsqueeze(0).expand(slot_dim, -1, -1, -1, -1).reshape(
                -1, n_heads, self.max_seq_length, dim_per_head)
            # assert all_attn_cache[i]["key"].size(0)
        ...
        if labels is not None:
            ...
            if self.graph_value_sup:
                graph_value_loss = nn.functional.nll_loss(masked_value_scores.view(-1, num_slot_labels),
                                                          labels.permute(2, 0, 1).reshape(-1), ignore_index=-1,
                                                          reduction='sum') / ds
