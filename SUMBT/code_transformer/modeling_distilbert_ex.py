...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids, start_offset=0):
        """
        Parameters
        ----------
        ...
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device) + start_offset # (max_seq_length)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, head_mask=None, attn_cache=None):
        """
        Parameters
        ----------
        ...
        if attn_cache:
            ex_k, ex_v = attn_cache["key"], attn_cache["value"]
            k = torch.cat([ex_k, k], dim=2)
            v = torch.cat([ex_v, v], dim=2)

            mask_reshp[-1] = k.size(2)
        ...
        if self.output_attentions:
            return (context, new_attn_cache, weights)
        else:
            return (context,new_attn_cache)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None, attn_cache=None):
        """
        Parameters
        ----------
        ...
        """
        # Self-Attention
        sa_output = self.attention(query=x, key=x, value=x, mask=attn_mask, head_mask=head_mask, attn_cache=attn_cache)
        if self.output_attentions:
            sa_output, sa_cache, sa_weights = sa_output  # (bs, seq_length, dim), (bs, n_heads, seq_length, seq_length)
        else:  # To handle these `output_attention` or `output_hidden_states` cases returning tuples
            assert type(sa_output) == tuple
            sa_output, sa_cache = sa_output
    ...
    def forward(self, x, attn_mask=None, head_mask=None, attn_cache=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(x=hidden_state, attn_mask=attn_mask, head_mask=head_mask[i],
                                         attn_cache=attn_cache[i] if attn_cache else None)
            hidden_state = layer_outputs[-1]
            all_sa_cache.append(layer_outputs[-2])
        ...
        outputs = (hidden_state, all_sa_cache)
        if self.output_hidden_states:
...
class DistilBertModel(DistilBertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, attn_cache=None, start_offset=0):
        r"""
    Return:
    ...
    Examples::
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embeddings(input_ids, start_offset=start_offset)  # (bs, seq_length, dim)
        tfmr_output = self.transformer(x=inputs_embeds, attn_mask=attention_mask, head_mask=head_mask, attn_cache=attn_cache)
...
class SimpleDialogSelfAttention(DistilBertPreTrainedModel):
    def __init__(self, config: DistilBertConfig, add_layer_norm: bool = False, add_residual: bool = False):
    ...
    def tie_position_embedding(self, position_embedding):
        self.position_embeddings.weight = position_embedding.weight
    ...
    def forward(self, hidden, attention_mask=None):
        ...
        sa_hidden = self.sa(hidden, hidden, hidden, extended_mask)[0]
        if self.add_layer_norm:
            if self.add_residual:
                sa_hidden = hidden + sa_hidden
            sa_hidden = self.sa_layer_norm(sa_hidden)
