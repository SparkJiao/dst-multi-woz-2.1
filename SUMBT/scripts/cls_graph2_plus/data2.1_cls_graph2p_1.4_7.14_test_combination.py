...
for i in range(10, 13):
    ...
    # predict_dir += '_mask_self'
    # cur_cmd = cmd + f' --test_mode {test_mode} --mask_self --predict_dir {predict_dir} '
    # run_cmd(cur_cmd)
...
for k, v in slot_idx.items():
    slot_res = v
    predict_dir = f'{output_dir}/test_mode_3_slot_res_{k}'
    cur_cmd = cmd + f' --test_mode {3} --slot_res {slot_res} --predict_dir {predict_dir} '
    run_cmd(cur_cmd)
