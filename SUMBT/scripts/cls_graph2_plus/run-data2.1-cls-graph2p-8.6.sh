...
#output_dir=exp-multiwoz/data2.1-cls-graph2p/v8.0-vsup=0.9
output_dir0=exp-multiwoz/data2.1-cls-graph2p/v8.6/s0  # two-stage training
target_slot='all'
nbt='graph2_p'
...
python code/main-multislot-share-5-newcls.py --do_train --do_eval --num_train_epochs 10 --data_dir data/multiwoz2.1_5 \
--bert_model bert-base-uncased --do_lower_case --bert_dir $bert_dir --task_name bert-gru-sumbt \
--nbt $nbt --output_dir $output_dir0 --target_slot $target_slot --warmup_proportion 0.1 --learning_rate 5e-4 \
--train_batch_size 8 --eval_batch_size 8 --distance_metric product --patience 10 \
...
--max_loss_scale 256 --value_embedding_type mean --sa_add_layer_norm --graph_attn_type 1 --graph_attn_head 12 --graph_add_layer_norm \
--fix_bert --per_eval_steps 1000

output_dir1=exp-multiwoz/data2.1-cls-graph2p/v8.6/s1

python code/main-multislot-share-5-newcls.py --do_train --do_eval --num_train_epochs 5 --data_dir data/multiwoz2.1_5 \
--bert_model bert-base-uncased --do_lower_case --bert_dir $bert_dir --task_name bert-gru-sumbt \
--nbt $nbt --output_dir $output_dir1 --target_slot $target_slot --warmup_proportion 0.2 --learning_rate 5e-5 \
...
--cls_type 0 --extra_nbt --graph_value_sup 0.9 --attn_head 12 --extra_nbt_attn_head 12 --diag_attn_hidden_scale 1.0 \
--max_loss_scale 256 --value_embedding_type mean --sa_add_layer_norm --graph_attn_type 1 --graph_attn_head 12 --graph_add_layer_norm \
--pretrain $output_dir0/pytorch_model.bin